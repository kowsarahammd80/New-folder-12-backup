...
const MyProductList = () => {
  ...
  useEffect(() => {
    fetch(`https://car-sell-buy-server-kowsarahammd80.vercel.app/products/${user?.email}`)
      .then(res => res.json())
  ...
  //   queryKey: ["repoData"],
  //   queryFn: () =>
  //     fetch(`https://car-sell-buy-server-kowsarahammd80.vercel.app/productsData/${user?.email}`)
  //       .then((res) => res.json())
  ...
  // const { refetch } = useQuery({
  //   queryKey: ['myProducts'],
  //   queryFn: () => fetch(`https://car-sell-buy-server-kowsarahammd80.vercel.app/products/${user?.email}`)
  //     .then(res => res.json())
  //     .then(data => {
  //       setMyPoroduts(data)
  //       refetch()
  ...
  const handleProductDelete = (id) => {
    ...
    // console.log(id)
    if (proceed) {
      fetch(`https://car-sell-buy-server-kowsarahammd80.vercel.app/productsData/${id}`, {
        method: "DELETE"
      })
        .then(res => res.json())
        .then(data => {
